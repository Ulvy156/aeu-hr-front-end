  ...
  <div class="space-y-6">
    ...
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <p class="mt-0.5 text-sm text-slate-500">View available leave balances for the selected year.</p>
      </div>
    </div>
    ...
    <template v-else-if="balanceData">
      ...
      <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-base font-semibold text-slate-900">{{ balanceData.employee.full_name }}</p>
          ...
          <template v-else>
            ...
            <div class="mt-3 h-1.5 rounded-full bg-gray-100 overflow-hidden">
              <div
                ...
                :class="getUsedPercent(balance) >= 80 ? 'bg-red-400' : `bg-${getBalanceColor(balance.leave_type)}-500`"
                :style="{ width: `${getUsedPercent(balance)}%` }"
              />
            </div>
            <p class="text-xs text-slate-400 mt-1">{{ getUsedPercent(balance) }}% used · {{ balance.rule }}</p>
          </template>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-700">
        Only approved leave requests reduce annual and sick balances. Balances are calculated dynamically by the backend.
      </div>
    </template>
